...
const App = () => {
  ...
  const handleDeleteContact = id => {
    ...
    if (window.confirm(`Delete ${contactToDelete.name} ?`)) {
      console.log(`Delete ${contactToDelete.id} now!!!!!!`)
      contactService
        .deleteContact(contactToDelete.id)
        .then(() => {
          setNotificationMessage(`The contact ${contactToDelete.name} has been deleted on the server`)
          setTimeout(() => {setNotificationMessage(null)}, 7000)  
  ...
  return (
    <div>
      ...
      <h2>Numbers</h2>
      <Persons persons={contactsToShow} handleDelete={handleDeleteContact} />
